...
const Cafes = () => {
  ...
  const filteredRestarurant = cafeData.filter((cafe) => {
    return (
      (searchName
        ? cafe.cafeName.toLowerCase().includes(searchName.toLowerCase())
        : true) &&
      (searchCategory
        ? cafe.thana.toLowerCase().includes(searchCategory.toLowerCase())
        : true)
    );
  });
  return (
    ...
    ${searchName ? "-top-3 scale-[0.9] left-2 text-[#3B9DF8]" : "top-3.5"}`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
        {filteredRestarurant.length > 0 ? (
          filteredRestarurant.map((item) => {
            return (
              <div
                key={item.id}
              ...
              >
                ...
                <div className="px-5 py-4 space-y-2">
                  ...
                  <p className="text-gray-600 text-sm">
                    🍴 <strong> তথ্য:</strong> {item.cafeInfo}
                  </p>
                  <p className="text-gray-600 text-sm">
                    🍽️ <strong>খাবারের আইটেম:</strong>{" "}
                    {item.foodItems.join(", ")}
                  </p>
                  <p className="text-gray-600 text-sm">
